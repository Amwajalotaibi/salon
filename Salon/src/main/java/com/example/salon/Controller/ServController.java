...
import com.example.salon.ApiResponse.ApiResponse;
import com.example.salon.Model.Serv;
import com.example.salon.Service.ServService;
...
public class ServController {
    ...
    public ResponseEntity updateServ(@Valid @RequestBody Serv serv, @PathVariable Integer id){
        servService.updateServ(serv, id);
        return ResponseEntity.status(200).body("Service Updated");
    }

    @DeleteMapping("/delete/{id}")
    public ResponseEntity deleteServ(@PathVariable Integer id){
        servService.deleteDServ(id);
